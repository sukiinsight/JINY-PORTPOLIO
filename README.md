...
3. `Run workflow` 클릭
4. workflow가 실행 중일 때 Telegram에서 [@Jiny_portf_bot](https://t.me/Jiny_portf_bot)에게 `test` 메시지 전송
5. Notion 데이터베이스에 새 페이지가 생겼는지 확인

이 테스트 workflow는 약 10분 동안만 봇을 켭니다. 계속 사용하는 운영 봇은 Render, Railway, Fly.io 같은 서버에 배포해야 합니다.
